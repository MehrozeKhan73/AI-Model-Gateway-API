...
# 检查创建是否成功
if echo "$CREATE_RESPONSE" | grep -q "success.*true\|data.*instanceId\|添加实例成功\|更新实例成功"; then
    echo "✓ 测试实例创建成功"
else
    echo "✗ 测试实例创建失败"
...
# 检查更新是否成功
if echo "$UPDATE_RESPONSE" | grep -q "success.*true\|data.*instanceId\|编辑成功"; then
    echo "✓ 实例配置更新成功"
else
    echo "✗ 实例配置更新失败"
